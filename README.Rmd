...
The plot below illustrates the number of detections that could be expected under various sampling intensity and population abundance scenarios. This figure illustrates that it is highly likely we would obtain at least one (and often more than one) detection under all the scenarios.
...
n_det_plot <- ggplot(all_results)+
  ...
  geom_text(data = result_summary,aes(x = 10, y = 150, label = paste0("Prob. of at least 1 detection = ",signif(result_summary$detected_species,2))),hjust=0, size = 2)+
  theme_bw()+
